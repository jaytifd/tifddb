...
from camp.views import (
    ...
    generate_email_html,
    get_membership_info,
    generate_cart_from_registration,
)
...
from camp.models import (
    ...
    CampRegistrationTypes,
    MembershipPayments,
    MembershipPerson,
    MembershipReport,
    PAYMENT_TYPES,
    REGISTRATION_PAID_STATUS,
)
...
from .forms import (
    ...
    MyCampHousingTypesFormSet,
    MyCampRegistrationTypesFormSet,
    PaymentAddForm,
)
...
def homepage(request):
    ...
    registration_status_options = CampRegistrationStatus.objects.all()
    payments_waiting_for_deposit = (
        MembershipPayments.objects.filter(waiting_for_deposit__exact=1)
        .filter(date_recd__year=thisyear)
        .select_related("registration")
        .select_related("membership_person")
    )

    c = (
        CampCamper.objects.filter(registration__year=thisyear)
        .order_by("-pk")
...
def refund_report(request):
    ...
    thisyear = get_thisyear(request)
    payments = (
        MembershipPayments.objects.filter(created_at__year=thisyear)
        .filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS)
        .filter(registration__registration_source=0)
        .filter(registration__payment_type="paypal")
        .filter(registration__id__gt=10664)
    )
...
def report_by_slug_render(
    ...
    thisyear,
    page_title="",
    template="registrar/reports_generic.html",
    hidden_fields=("registration",),
    extra={},
):
...
def report_by_slug(request, report_by_slug):
    ...
    if report_by_slug == "camproster":
        ...
        result_dict = (
            ...
            .filter(adult_or_child__iexact="adult")
            .values(*searchfields)
            .order_by("last_name")
            .filter(registration__registration_source=0)
            .filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS)
        ...
        hidden_fields = [
            ...
            "last_name",
            "year",
            "registration__address1",
            "registration__address2",
            "registration__city",
            "registration__state",
        ...
        return report_by_slug_render(
            ...
            thisyear,
            hidden_fields=hidden_fields,
            template="registrar/reports_camproster.html",
        )
    ...
    #################T-SHIRT ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(Q(free_t_shirt=1) | Q(t_shirt_type_id__gt=1))
            .filter(registration__year__icontains=thisyear)
            .values(*searchfields)
            .filter(
                Q(first_name__icontains=search)
        ...
        return report_by_slug_render(
            ...
            thisyear,
            hidden_fields=hidden_fields,
            template="registrar/reports_tshirt.html",
            extra=group_dict,
        )
    ...
    #################DVD ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(Q(dvd__exact=1))
            .filter(registration__year__icontains=thisyear)
            .values(*searchfields)
            .filter(
                Q(first_name__icontains=search)
        ...
        return report_by_slug_render(
            ...
            thisyear,
            hidden_fields=hidden_fields,
            template="registrar/reports_dvd.html",
        )
    ...
    #################CAMP STAFF ################
        ...
        result_dict = (
            ...
            .filter(registration__year__exact=thisyear)
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
                | Q(diet_details__icontains=search)
            )
            .values(*searchfields)
            .filter(registration__registration_source=0)
            .filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS)
        ...
        return report_by_slug_render(
            request, report_by_slug, fields, result_dict, thisyear
        )
    ...
    #################LINEN ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(Q(need_linen__exact=1))
            .filter(registration__year__exact=thisyear)
    ...
    #################DIETS ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(Q(diet__exact=1) | Q(diet_details__isnull=False))
            .filter(registration__year__exact=thisyear)
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
    ...
    #################FAMILY PROGRAM ################
        ...
        result_dict = (
            CampCamper.objects.filter(
                Q(family_program__exact=1) | Q(adult_or_child__iexact="child")
            )
            .filter(registration__year__icontains=thisyear)
            .filter(
                Q(first_name__icontains=search)
    ...
    #################ADDRESS LABELS ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.values(*searchfields)
            .filter(registration__year__exact=thisyear)
            .order_by("last_name")
            .filter(registration__registration_source=0)
            .filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS)
        ...
        return report_by_slug_render(
            ...
            result_dict,
            thisyear,
            template="registrar/reports_addresslabels.html",
        )
    ...
    #################NAME BADGES ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(registration__year__icontains=thisyear)
            .values(*searchfields)
            .filter(
                Q(first_name=search)
                | Q(last_name=search)
    ...
    #################HOUSING ################
        ...
        result_dict = (
            ...
            .filter(
                ...
                | Q(last_name__icontains=search)
                | Q(registration__city__icontains=search)
                | Q(housing_assigned__icontains=search)
            )
            .filter(registration__registration_source=0)
            .filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS)
        )
    ...
    ################# SINGING  ################
        ...
        result_dict = (
            CampCamper.objects.filter(
                Q(singing__exact=1) | Q(instruments__isnull=False)
            )
            .filter(registration__year__icontains=thisyear)
            .values(*searchfields)
            .filter(
    ...
    #################MUSICIANS  /  BAND ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(Q(band__exact=1) | Q(instruments__isnull=False))
            .filter(registration__year__icontains=thisyear)
            .values(*searchfields)
            .filter(
                Q(first_name__icontains=search)
    ...
    #################REFUND REPORT ################
        ...
        result_dict = MembershipPayments.objects.filter(Q(refund_amt__gt=0)).filter(
            registration__year__icontains=thisyear
        )
        ...
        if request.GET.get("csv"):
            ...
            for r in result_dict:
                mylist = [r.registration.id, r.registration, r.refund_amt, r.notes]
    ...
    #################LIKELY CAMPERS  ################
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            CampCamper.objects.filter(registration__year__gte=thisyear)
            .values(*searchfields)
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
    ...
    #################DONATIONS ################
        ...
        fields = [
            ...
            {
                "registration__registration_source": "registration_source"
        ...
        for f in fields:
            searchfields += f
        result_dict = (
            MembershipPayments.objects.filter(
            ...
            .filter(created_at__year=thisyear)
            .filter(registration_id__gte=1)
            .values(*searchfields)
        )
        i = 0
        ...
        for r in result_dict:
            if r["pp_ipn_name"]:
                ...
                campers = CampCamper.objects.filter(
        ...
        if fields:
            fields.append({"": "donation source"})
        hidden_fields = [
            "registrationid",
            "donations_report",
            "registration__registration_source",
        ] + keys_to_remove
        return report_by_slug_render(
            request,
            report_by_slug,
    ...
    #################POSTCARD REPORT ################
        ...
        # result_dict=CampCamper.objects.filter(Q(first_name__icontains=search) | Q(last_name__icontains=search) | Q(registration__city__icontains=search)).filter(registration__registration_status_id__in=REGISTRATION_PAID_STATUS).values(*searchfields).order_by('-registration__year')
        result_dict = (
            CampCamper.objects.filter(Q(membership_valid_to__year__gte=thisyear))
            .exclude(adult_or_child__exact="child")
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
        ...
        if request.GET.get("table"):
            ...
            return report_by_slug_render(
                ...
                thisyear,
                extra=addresses,
                template="registrar/reports_postcards_addresslabels.html",
            )
    ...
    #################LIFETIME MEMBERS ################
        ...
        result_dict = (
            MembershipPerson.objects.filter(Q(membership_valid_to__gte=now))
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
    ...
    ###################TIFD MEMBERS#########
        ...
        report_by_slug == "members_in_good_standing"
        or report_by_slug == "members_all"
        or report_by_slug == "members_expiring"
    ):
        ...
        result_dict_lifetime_expired = (
            MembershipPerson.objects.filter(Q(membership_valid_to__year__lte=now.year))
        ...
        result_dict_camper_valid = (
            CampCamper.objects.filter(membership_valid_to__gt=now)
            .filter(
                Q(first_name__icontains=search)
                | Q(last_name__icontains=search)
                | Q(registration__city__icontains=search)
        ...
        result_dict_camper_expired = (
            CampCamper.objects.filter(membership_valid_to__year__lte=now.year)
        ...
        result_dict_valid = list(result_dict_camper_valid) + (
            list(result_dict_lifetime_valid)
        )
        result_dict_valid = sorted(
            result_dict_valid, key=lambda x: x["last_name"] or ""
        )

        result_dict_expired = list(result_dict_camper_expired) + (
            list(result_dict_lifetime_expired)
        )
        result_dict_expired = sorted(
            result_dict_expired, key=lambda x: x["last_name"] or ""
        )
            ...
            if index_string not in dupe_check:
                ...
                if member:
                    ...
                    if (report_by_slug == "members_expiring") and (
                        member[0].membership_valid_to.year
                        < datetime.datetime.strptime(str(thisyear), "%Y").year
                        or member[0].membership_valid_to.year > now.year
                    ):
                        p(
                            "pass:",
                            member[0],
        ...
        return report_by_slug_render(
            request, report_by_slug, fields, result_dict_no_dupes, thisyear
        )
        ...
        if request.POST.get("payments"):
            ...
            if request.POST.get("save"):
                for x in payments:
                    ...
                    saved = True
            payments = (
                MembershipPayments.objects.filter(date_recd__icontains=thisyear)
                .filter(pk__in=payments)
                .select_related("registration")
                .select_related("membership_person")
            )
        else:
            ...
            messages.error(
                request,
                "Error - no payments selected.  Please select a payment by checking the box to the left of the payment.",
            )
...
def adjustment_add(request):
    if request.POST.get("adjustment") and request.POST.get("registration_id"):
        registration = get_object_or_404(
            CampRegistration, id=request.POST.get("registration_id")
        )
        if registration:
...
def itemize_payment(registration, payment=None):
    ...
    if registration is None and payment is None:
        ...
        for c in campers:
            if c.need_linen:
                payment_fields["gfc_linens"] += CampPrices.get_price("linen")
            if c.t_shirt_type_id:
                payment_fields["t_shirt_fee"] += c.t_shirt_type.price
            if c.dvd:
                payment_fields["dvd_fee"] += CampPrices.get_price("dvd")
            ...
            if c.registration.registration_source == 0:
                if c.adult_or_child == "adult" and c.join_tifd == 1:
                    payment_fields["membership_fee"] += CampPrices.get_price(
                        "membership"
                    )
            ...
            if c.registration_type.slug == "membership":
                if "ifetime" in c.registration_type.description:
                    ...
                    payment_fields["membership_fee"] += (
                        c.registration_type.price * c.membership_years
                    )
        ...
        if registration.paypal_fee_reimburse_flag:
            payment_fields[
                "paypal_fee_reimburse_fee"
            ] += registration.paypal_fee_reimburse_fee
        if registration.rebate:
            payment_fields["membership_fee"] = (
                payment_fields["membership_fee"] + registration.rebate.price
            )
            p(
                "applied rebate",
                registration.rebate.price,
                "membership fee:",
                payment_fields["membership_fee"],
            )
...
def payments_add(request, payment_id=False):
    ...
    if request.GET.get("registration_id"):
        try:
            ...
            registration = CampRegistration.objects.get(
                pk=request.GET.get("registration_id")
            )
            cart, cart_total = generate_cart_from_registration(
                registration.id, save=False
            )
            discount_list, discount_total = get_discount(registration.id)
            remaining_balance = cart_total
        except Exception as e:
            p(
                'CRITICAL ERROR: registration not found: "',
                request.GET.get("registration_id"),
                '"',
                request,
                e,
            )
            messages.error(
                request,
                "Registration " + request.GET.get("registration_id") + " not found! !",
            )
    ...
    if payment_id is False:
        ...
        new_payment_form.fields["who_has_possession"].initial = str(
        ...
        if registration:
            ...
            previous_payments = MembershipPayments.objects.filter(
                registration=registration
            )  ##but not the current payment?
    ...
    if request.method == "POST":
        ...
        if new_payment_form.has_changed():
            for c in new_payment_form.changed_data:
                p(
                    request.user,
                    'DEBUG: new_payment_form form field %s went from "%s" to "%s"'
                    % (c, new_payment_form[c].initial, new_payment_form[c].data),
                )
            ...
            if new_payment_form.is_valid():
                ...
                messages.success(
                    request, "Payment ID " + str(f.id) + " saved successfully!"
                )
                ...
                if request.POST.get("remaining_balance") == "0.00":
                    if registration:
                        ...
                        for c in campers:
                            ...
                            p(
                                "remaining balance is zero - renewing membership for",
                                c,
                                "until:",
                                valid_to,
                            )
                        ...
                        return HttpResponseRedirect(
                            reverse("registrar:approve", args=(registration.id,))
                        )
...
def payments_report(request):
    ...
    payments = (
        MembershipPayments.objects.filter(date_recd__gte=start_date_obj)
        .filter(date_recd__lte=end_date_obj)
        .filter(id__gte=1)
        .filter(who_has_possession__icontains=search)
        .select_related("membership_person")
        .select_related("registration")
    )
    ...
    for pmt in payments:
        if pmt.id > 0:
            ...
            if pmt.paypal_fee:
                payments_paypalfee += pmt.paypal_fee
            ...
            if pmt.registration_id == 1 or pmt.registration_id is None:
                ...
                pmt.registration
            ):  # a payment attached to a camp/membership registration
                if pmt.registration.registration_status_id in REGISTRATION_PAID_STATUS:
                    # only report on PAID registrations

                    if pmt.registration.cart_total:
                        registrations_cart_total += pmt.registration.cart_total
                    ...
                    if (
                        pmt.registration.registration_status_id == 11
                        and pmt.registration.year == 2021
                    ):
                        refunds_total -= abs(pmt.refund_amt)
                ...
                # legacy payments
                p(
                    "PAYMENT WITHOUT REGISTRATON ID!",
                    pmt.registration_id,
                    pmt.id,
                    pmt.__dict__,
                )
    ...
    payments_net = (
        payments_total - payments_paypalfee
    )  # -refunds_total  refunds are already subtracted from gross

    return render(
        request,
        "registrar/payments_report.html",
        {
            "payments": payments,
            "payments_total": payments_total,
...
def approve(request, registration_id):
    ...
    for c in campers:
        c.temp_membership_valid_from, c.temp_membership_valid_to = (
            renew_tifd_membership(c, save=False)
        )

    if registration.registration_source == 0:
        email_content = generate_email_html(
            registration, template_slug="registration_approved"
        )
    elif registration.registration_source == 1:
        email_content = generate_email_html(
            registration, template_slug="membership_confirmed"
        )

    else:
        p("REGISTRATION SOURCE WAS NOT ZERO OR ONE -- THIS SHOULD NOT HAPPEN")
        email_content = generate_email_html(
            registration, template_slug="membership_approved"
        )

    approve_registration_form.initial["registrar_approval_note"] = email_content[
        "intro_message"
    ]

    return render(
        request,
        "registrar/approve.html",
        {
            "registration": registration,
            "cart": cart,
            "previous_payments": previous_payments,
...
def membership_report(request):
    ...
    p("search'", search)
    membership_person = (
        MembershipPerson.objects.filter(
            Q(last_name__icontains=search) | Q(first_name__icontains=search)
        )
        .filter(membership_valid_to__gte=datetime.date(now.year, now.month, now.day))
        .select_related("membership_address")
    ...
    return render(
        request,
        "registrar/membership_report.html",
        {
            "reports": MembershipReport.objects.all,
            "years": years,
...
def registrar(request):
    ...
    if paid == "on":
        campers = campers.filter(
            registration__registration_status__in=REGISTRATION_PAID_STATUS
        )
        myregistrations = myregistrations.filter(
            registration_status__in=REGISTRATION_PAID_STATUS
        )
        sendmsg = True
        msg.add("paid")
    if camponly == "on":
    ...
    all_payments = MembershipPayments.objects.filter(
        registration_id__in=registrations_set
    ).select_related("membership_person")
    ...
    registration_header_info["outstanding"] = registration_header_info["total_owed"] - (
        registration_header_info["total_gross"]
    )
...
def donationletter(request, registration, pdf_only=False, trigger=1):
    ...
    campers = CampCamper.objects.filter(registration=registration).filter(
        adult_or_child__icontains="adult"
    )
    ...
    if request and pdf_only is False:
        ...
        response["Content-Disposition"] = (
            f"inline; filename={registration.year}{registration.id}.pdf"
        )
...
def renew_tifd_membership(
    camper, save=True, thisyear=int(datetime.datetime.now().year)
):
    """
    ...
    p(
        "inside renew_tifd_membership. save:",
        save,
        "camper:",
        camper,
        " years:",
    ...
    if camper.join_tifd == 1:
        if camper.membership_years is None:
            p(
                "join TIFD is 1 but membership years is null or zero - this is probably a bug",
                camper.membership_years,
            )
...
def view(request, registration_id):
    campers = CampCamper.objects.filter(registration__exact=registration_id).order_by(
        "adult_or_child", "id"
    )
    campers_list = campers.values()
    registration = CampRegistration.objects.get(pk=registration_id)
    payments_list = MembershipPayments.objects.filter(
...
def payments_quick(request, registration_id):
    ...
    if Decimal(gross_amt) != Decimal(qp_total):
        p(
            "ERROR - invalid quick payment! gross_amt:",
            gross_amt,
            "qp_total:",
            qp_total,
            "itemizations",
        ...
        messages.error(
            request,
            f"ERROR - bad quick payment! gross_amt: {gross_amt} qp_total: {qp_total}",
        )
        new_payment.delete()
        return HttpResponseRedirect(reverse("registrar:homepage"))
    new_payment.who_has_possession = str(
        request.user.first_name + " " + str(request.user.last_name)
    )
    ...
    p(
        registration_id,
        request.user,
        "quick payment done id:",
        new_payment.id,
        "qp_total:",
        qp_total,
        "cart_total",
        registration.cart_total,
        "gross_amt:",
        gross_amt,
        "itemizations:",
        itemizations.items(),
...
def campconstants(request):
    ...
    thisyear = get_thisyear(request)
    camp_registration_types_formset = modelformset_factory(
        CampRegistrationTypes,
        fields=["description", "price"],
        formset=MyCampRegistrationTypesFormSet,
        extra=0,
    )
    camp_housing_types_formset = modelformset_factory(
        CampHousingTypes,
        fields=["description", "price"],
        formset=MyCampHousingTypesFormSet,
        extra=0,
    )
    camp_dates_formset = modelformset_factory(
        CampDates, fields=["description", "date"], extra=0
    )

    if request.method == "POST":
        form_open = CampDates.objects.get(slug="form_open").date
        p("Form open:", form_open, datetime.datetime.now().date())
        # constants_form = CampConstantsForm(request.POST, prefix="constants")

        camp_dates_formset = camp_dates_formset(request.POST)
        registration_formset = camp_registration_types_formset(
            request.POST, prefix="registration"
        )
        housing_formset = camp_housing_types_formset(request.POST, prefix="housing")

        p(
            f"constants:{camp_dates_formset.has_changed()} reg:{registration_formset.has_changed()} housing:{housing_formset.has_changed()}"
        )
        ...
        if registration_formset.has_changed() or housing_formset.has_changed():
            if now.date() >= form_open:
                p(
                    f"Error - tried to edit prices after form opened. open:{form_open} now:{now.date()}"
                )
                messages.add_message(
                    request,
                    messages.ERROR,
                    "You can't change prices after the form opens!",
                )
                return redirect("registrar:campconstants")
            ...
            if registration_formset.is_valid() and housing_formset.is_valid():
                ...
                messages.add_message(
                    request,
                    messages.ERROR,
                    registration_formset.errors,
                    housing_formset.errors,
                )
                p(
                    f"registration errors:{registration_formset.errors} housing errors:{housing_formset.errors}"
                )
                return redirect("registrar:campconstants")
    ...
    for r in result_dict:
        ...
        if name not in members:
            ...
            p(
                "duplicate member",
                name,
                r["registration__address1"],
                r["registration"],
                r["registration__year"],
            )
    ...
    for r in result_dict:
        if r["registration__address1"]:
            ...
            if address in addresses:
                ...
                addresses[address]["registration__address2"] = r[
                    "registration__address2"
                ]
                if r["registration__city"]:
                    addresses[address]["registration__city"] = r[
                        "registration__city"
                    ].title()
                if r["registration__state"]:
                    addresses[address]["registration__state"] = r[
                        "registration__state"
                    ].upper()
                addresses[address]["registration__zip"] = r["registration__zip"]
                if (
                    r["registration__country"]
                    and r["registration__country"].lower() not in country_exclude
                ):
                    addresses[address]["registration__country"] = r[
                        "registration__country"
                    ]
    return addresses
...
def campcounts(request):
    ...
    return render(
        request,
        "registrar/reports_campcounts.html",
        {
