...
from camp.models import CampRegistration, CampRegistrationTypes, CampCamper
from .forms import RegistrationFormset_edit, RegistrationFormset_new, PersonFormset

# from .forms import *
# from .models import *

years = (
    CampRegistration.objects.order_by("-year")
    .values(
        "year",
    )
    .distinct()
...
def create(request, registration_id=None):
    ...
    edit_view = False
    renewid = None
    registration_options = CampRegistrationTypes.objects.filter(active=True).filter(
        slug__exact="membership"
    )
    ...
    p(
        "Example membership renewal facts begin/end",
    ...
    if request.method == "POST":
        ...
        regformform_person = PersonFormset(
            ...
            for form in regformform_person:
                if form.has_changed():
                    for c in form.changed_data:
                        p(
                            '[%s] adult form field %s went from "%s" to "%s"'
                            % (str(registration.pk), c, form[c].initial, form[c].data)
                        )
        ...
        if registrationform.is_valid() and regformform_person.is_valid():
            ...
            if edit_view is False:
                ...
                if (
                    ...
                    or regformform_person[0].cleaned_data == {}
                ):
                    ...
                    return render(
                        request,
                        "membership/registration_form.html",
                        {
            ...
            if regformform_person.is_valid():
                ...
                messages.success(
                    request,
                    "Saved Successfully! Registration ID: " + str(registration_pk),
                )
