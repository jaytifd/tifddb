...
from camp.models import CampRegistration, MembershipPayments, CampCamper
...
def invalid_ipn(sender, **kwargs):
    ...
    ipn_obj = sender
    p(
        "INVALID IPN RECEIVED — possible fraud or misconfiguration.",
    ...
    p("Stack trace at invalid IPN:\n", stack)
    raise Exception(
        f"Invalid PayPal IPN received — invoice={ipn_obj.invoice} "
        f"gross={ipn_obj.mc_gross} receiver={ipn_obj.receiver_email} "
        f"txn_id={ipn_obj.txn_id}"
    )
...
def show_me_the_money(sender, **kwargs):
    ...
    if ipn_obj.payment_status == ST_PP_REVERSED:
        p(
            "Here inside REVERSED IPN",
            ipn_obj,
            "invoice",
            ipn_obj.invoice,
            type(ipn_obj.invoice),
            ipn_obj.mc_gross,
        )
        ...
        if (
            ipn_obj.receiver_email != "jay-facilitator@spaz.org"
            and ipn_obj.receiver_email != "payments@tifd.org"
        ):
            p("PayPal callback had wrong receiver email:", ipn_obj.receiver_email)
            return False
        registration = get_object_or_404(
            CampRegistration, transaction_id=ipn_obj.invoice
        )
        p(
            "Here inside REFUND IPN2",
            ipn_obj,
            "registration",
            registration,
            "invoice",
        ...
        if registration:
            ...
            if mypayment:
                ...
                mypayment.save()
                p(
                    "Here inside REFUND PAYMENT SAVED..",
                    ipn_obj,
                    "registration",
                    registration,
                    "payment",
        ...
        registration = CampRegistration.objects.filter(transaction_id=ipn_obj.invoice).first()
        if not registration:
            p("IPN invoice not found, treating as anonymous donation. invoice:", ipn_obj.invoice)
            registration = CampRegistration.objects.get(pk=1)

        p(
            "Incoming PayPal callback - invoice number:",
            ipn_obj.invoice,
            "registration:",
            registration,
        )
        ...
        payment.save()
        p(
            "PayPal callback received! Payment recorded: status=IPN valid, invoice=",
            ipn_obj.invoice,
            " gross=",
            ipn_obj.mc_gross,
            "cart total",
        ...
        if registration.pk == 1:
            p(
                "PayPal IPN registration 1 - anonymous donation payment_id:",
                payment.id,
                "ipn_id:",
                ipn_obj.id,
                "item name:",
            ...
            if ipn_obj.item_name == "Virtual Camp 2020":
                p(
                    "PayPal IPN registration 1 - anonymous donation - ",
                    payment.gross_amt,
                    "sent to camp_fee",
                )
                ...
                p(
                    "PayPal IPN - no fund found for item_name:",
                    ipn_obj.item_name,
                    "sending ",
                    payment.gross_amt,
                    "to general fund. payment_id:",
                    payment.id,
                    "ipn_id:",
                    ipn_obj.id,
                )
            ...
            if pdf:
                ...
                if not emailconfirmation(registration, request, template):
                    p(
                        "EMAIL SEND FAILED donation letter signals.py regid:",
                        registration.id,
                    )
            return True
        ...
        #### CAMP / MEMBERSHIP REGISTRATION FULLY PAID #######
            ...
            p(
                "PayPal callback received! Finished updating registration status. registration_id:",
                registration.pk,
                "status:",
            ...
            payment.save()
            p(
                "PayPal callback received! Finished itemizing payment.  registration_id:",
                registration.pk,
                "status:",
                registration.registration_status,
                "payment_id:",
        ...
        #######CAMP / MEMBERSHIP REGISTRATION __NOT__ FULLY PAID
            ...
            p(
                "PayPal callback received! Cart total was not equal to payment. invoice=",
                ipn_obj.invoice,
                " gross=",
                ipn_obj.mc_gross,
                "cart total",
