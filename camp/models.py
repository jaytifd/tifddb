...
class MembershipRegistrationTypes(models.Model):
    ...
    active = models.BooleanField(
        "Include on regform?", max_length=1, default=1, blank=False, null=False
    )
...
class CampRegistrationTypes(models.Model):
    ...
    slug = models.CharField(
        'slug - either "registration" or "membership"',
        max_length=255,
        blank=False,
        null=False,
    )
...
class CampRegistration(models.Model):
    ...
    address1 = models.CharField(
        "Mailing address - street", max_length=255, blank=False, null=True
    )
    address2 = models.CharField(
        "Mailing address - apt/unit", max_length=255, blank=True, null=True
    )
    city = models.CharField(max_length=255, blank=False, null=True)
    state = models.CharField(
        max_length=255, blank=False, null=True, default="", choices=YOUR_STATE_CHOICES
    )
    ...
    donation_bobbi_gillotti = models.DecimalField(
        "Bobbi Gillotti fund donation",
    ...
    donation_floor_fund = models.DecimalField(
        "Floor fund donation",
        max_digits=7,
        validators=[MinValueValidator(Decimal("0.00"))],
        decimal_places=2,
    ...
    donation_camp_fund = models.DecimalField(
        "Floor fund donation",
        max_digits=7,
        validators=[MinValueValidator(Decimal("0.00"))],
        decimal_places=2,
    ...
    donation_live_music = models.DecimalField(
        "Live music donation",
        max_digits=7,
        validators=[MinValueValidator(Decimal("0.00"))],
        decimal_places=2,
    ...
    donation_tifd = models.DecimalField(
        "TIFD fund donation",
        max_digits=7,
        validators=[MinValueValidator(Decimal("0.00"))],
        decimal_places=2,
    ...
    donation_chuck = models.DecimalField(
        "Donation in remembrance of Chuck Roth",
        max_digits=7,
        validators=[MinValueValidator(Decimal("0.00"))],
        decimal_places=2,
        blank=True,
    ...
    email_confirmation_sent = models.BooleanField(
        "Email conf sent?", max_length=1, blank=True, null=True, default=0
    )
    ...
    cart_total = models.DecimalField(
        max_digits=7, decimal_places=2, blank=True, null=False, default=0
    )
    membership_fee_gross = models.DecimalField(
    ...
    paypal_gross = models.DecimalField(
        max_digits=7, decimal_places=2, blank=True, null=True
    )
    agreecheckbox = models.BooleanField(
        "Safety policy checkbox", blank=False, null=False
    )
    registrar_approval_note = models.TextField(
        "Registrar confirmation email", blank=True, null=True
    )
    rebate = models.ForeignKey(
        CampRebates,
        on_delete=models.DO_NOTHING,
        related_name="rebate_from_registration",
        blank=True,
        null=True,
    )
    ...
    year = models.IntegerField(
        blank=False, null=True, default=datetime.datetime.now().year
    )
    postmark = models.DateField(blank=True, null=True)
    registration_source = models.IntegerField(
        blank=False, null=False, default=0, choices=REGISTRATION_SOURCE
    )  ## 0=camp, 1=membership
    history = HistoricalRecords()
    campers = CampRegistrationStatus.objects.raw("SELECT * FROM camp_camper")
    paypal_fee_reimburse_flag = models.BooleanField(
        "I would like to compensate TIFD for the paypal fees associated with my registration.",
        default=False,
    )
    paypal_fee_reimburse_fee = models.DecimalField(
...
class MembershipAddress(models.Model):
    ...
    registration = models.ForeignKey(
        CampRegistration, on_delete=models.DO_NOTHING, blank=False, null=False
    )
    ...
    try:
        for x in all_persons:
            if x.membership_address_id in persons.keys():
                persons[x.membership_address_id] += (
                    ", " + str(x.first_name) + " " + str(x.last_name)
                )
            else:
                persons[x.membership_address_id] = (
...
class MembershipPerson(models.Model):
    ...
    membership_address = models.ForeignKey(
        MembershipAddress, on_delete=models.DO_NOTHING, blank=False, null=False
    )
    membership_type = models.ForeignKey(
        MembershipType, on_delete=models.DO_NOTHING, blank=False, null=False
    ...
    registration = models.ForeignKey(
        CampRegistration, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    registration_type = models.ForeignKey(
        MembershipRegistrationTypes, on_delete=models.DO_NOTHING, default=1
    )
...
class CampPrices(models.Model):
    ...
    @classmethod
    def get_description(cls, slug):
        try:
            return cls.objects.get(slug=slug).cart_description
        except cls.DoesNotExist:
            return ""

    @classmethod
    def get_price(cls, slug):
        try:
            return cls.objects.get(slug=slug).price
        except cls.DoesNotExist:
            return None
...
class Person(models.Model):
    ...
    registration_type = models.ForeignKey(
        ...
        on_delete=models.DO_NOTHING,
        related_name="registration_type_from_camper",
        default=1,
    )
    adult_or_child = models.CharField(
        max_length=255, blank=False, null=False, default="adult", choices=ADULT_OR_CHILD
    )
    ...
    mobility = models.BooleanField(
        "I use a mobility device such as wheelchair, walker, crutches, or cane.",
        blank=True,
        null=False,
        default=False,
    )
    mobility_details = models.CharField(
        "Mobility device details", max_length=255, blank=True, null=True
    )
    certification = models.BooleanField(
        "I am certified in CPR, first aid, or AED, and am willing to be called on for first response in an emergency at camp.",
        blank=True,
        null=False,
        default=False,
    )
    certification_details = models.CharField(
        "Certification details", max_length=255, blank=True, null=True
    )
    family_program = models.BooleanField(
        "Participating in family program?", blank=True, null=False, default=False
    )
    medical = models.BooleanField(
        "I or my child(ren) have allergies, medical conditions, or medications the camp safety team should be aware of.",
        blank=True,
        null=False,
        default=False,
    )
    medical_details = models.CharField(
        "Medical notes", max_length=255, blank=True, null=True
    )
    band = models.BooleanField(
        "I will play in the camp band.", blank=True, null=False, default=False
    )
    singing = models.BooleanField(
        "I am interested in attending singing classes.",
        blank=True,
        null=False,
        default=False,
    )
    ...
    diet = models.BooleanField(
        "Dietary restrictions", blank=True, null=False, default=False
    )
    diet_details = models.TextField("Dietary restrictions", blank=True, null=True)
    publish = models.BooleanField(
        "I consent to having my name, email address, and phone number published in TIFD's membership and Texas Camp rosters (we do not publish physical addresses for security reasons).",
        blank=True,
        null=False,
        default=False,
    )
    staff = models.BooleanField(
        "I am a member of TIFD's administrative staff (camp committee volunteer).",
        blank=True,
        null=False,
        default=False,
    )
    need_linen = models.BooleanField(
        "Check here to rent linens (sheets/towels) provided by GFC",
        blank=True,
        null=False,
        default=False,
    )
    staff_position = models.CharField(
        "Staff position(s)", max_length=255, blank=True, null=True
    )
    free_t_shirt = models.BooleanField(
        "Free t-shirt for staff members", blank=True, null=False, default=False
    )
    dvd = models.BooleanField(
        "I wish to order a dance review video, in which this year's teachers demonstrate the dances they taught. Digital access - TIFD will send a link after camp",
        blank=True,
        null=False,
        default=False,
    )
    share_housing = models.CharField(
        "I prefer to share housing with", max_length=255, blank=True, null=True
    )
    ...
    housing_type = models.ForeignKey(
        CampHousingTypes, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    t_shirt_type = models.ForeignKey(
        CampShirtTypes,
        on_delete=models.DO_NOTHING,
        verbose_name="I wish to order a commemorative camp t-shirt.",
        null=True,
        blank=True,
        default=1,
    )
    membership_years = models.IntegerField(
        blank=False,
        null=False,
        default=1,
    )
    join_tifd = models.BooleanField(
        "I would like to become a TIFD member or renew my membership ($15 for one year)",
        blank=False,
        null=False,
        default=True,
    )
    custom_registration_price = models.DecimalField(
        "Donation amount",
        max_digits=7,
        decimal_places=2,
        blank=True,
    ...
    custom_registration_discount = models.DecimalField(
        "Discount amount",
        max_digits=7,
        decimal_places=2,
        blank=True,
    ...
    def __str__(self):
        ...
        if self.last_name is None:
            self.last_name = ""
        return str(
            self.first_name + " " + self.last_name + " " + str(self.created_at.year)
        )
...
class CampCamper(Person):
    ...
    class Meta:
        ...
        ordering = [
            "-id"
        ]  # this ordering will affect how the form is rendered on the reg form
...
class CampCamper_adult(Person):
    registration = models.ForeignKey(
        CampRegistration,
        on_delete=models.DO_NOTHING,
        related_name="registration_from_adult",
    )
    registration_type = models.ForeignKey(
        CampRegistrationTypes,
        on_delete=models.DO_NOTHING,
        related_name="registration_type_from_adult",
        default=1,
    )  ### if you change the default, fix the camp/templates/camp/delete.html too

    if virtual_camp:
        housing_type = models.ForeignKey(
            CampHousingTypes,
            on_delete=models.DO_NOTHING,
            related_name="housing_type_from_adult",
            null=True,
            blank=True,
        )
    else:
...
class CampCamper_child(Person):
    adult_or_child = models.CharField(
        max_length=255, blank=False, null=False, default="child", choices=ADULT_OR_CHILD
    )
    registration = models.ForeignKey(
        CampRegistration,
        on_delete=models.DO_NOTHING,
        related_name="registration_from_child",
    )
    registration_type = models.ForeignKey(
        CampRegistrationTypes,
        on_delete=models.DO_NOTHING,
        related_name="registration_type_from_child",
        default=11,
    )
    history = HistoricalRecords()
...
class MembershipPayments(models.Model):
    ...
    gross_amt = models.DecimalField(
        "Gross", max_digits=10, decimal_places=2, blank=False, null=True
    )
    net_amt = models.DecimalField(
        "Net", max_digits=10, decimal_places=2, blank=False, null=True
    )
    refund_amt = models.DecimalField(
        "Refund", max_digits=10, decimal_places=2, blank=True, null=True
    )
    date_recd = models.DateField(
        "Date received",
        blank=False,
        null=False,
        default=datetime.datetime.now().strftime("%Y-%m-%d"),
    )
    who_has_possession = models.CharField(
        "Who has possession", max_length=255, blank=False, null=False
    )
    payment_type = models.IntegerField("Type", choices=PAYMENT_TYPES, default=1)
    waiting_for_deposit = models.BooleanField(
        "Waiting for deposit", null=False, blank=False, default=1
    )  # Field name made lowercase.
    ...
    paypal_fee = models.DecimalField(
        "PayPal fee", max_digits=10, decimal_places=2, blank=True, null=True
    )
    camp_fee = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    t_shirt_fee = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    shipping_fee = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    dvd_fee = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    gfc_linens = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    registration = models.ForeignKey(
        CampRegistration, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    membership_person = models.ForeignKey(
        MembershipPerson, on_delete=models.DO_NOTHING, blank=True, null=True
    )
    housing_fee = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    ...
    bobbi_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    floor_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    music_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    general_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    camp_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    chuck_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    texakolo_fund = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True
    )
    paypal_fee_reimburse_fee = models.DecimalField(
        "PayPal fee reimburse",
        max_digits=10,
        decimal_places=2,
        blank=True,
        null=True,
    ...
    def get_fields(self):
        return [
            ...
            for field in MembershipPayments._meta.fields
        ]
